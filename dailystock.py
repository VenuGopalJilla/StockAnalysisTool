...
def download_deliverables():
    ...
    path = os.path.join(os.getcwd(), "Data", "SP500_Stock")
    ...
    def extract_save(name):
        ...
        deli.to_csv(os.path.join(path, "deliverable"+".csv"), index=False)
...
def download_bhavcopy():
    ...
    def extract_save(name):
        ...
        bhav["DATE"] = today
        bhav.to_csv(os.path.join(path, "bhav.csv"), index=False)
    ...
    for index, row in result.iterrows():
        try:
            if os.path.exists(os.path.join(path, str(index)+".csv")):
                ...
                stk.to_csv(os.path.join(path, str(index)+".csv"), index=False)
        except:
            pass
...
result.to_csv(os.path.join(path, "previousdaystockdetails.csv"), index=False)
update_files()